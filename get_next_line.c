...
/*   Updated: 2023/11/28 12:52:36 by jajuntti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*concatenate(char *new_str, char *heap_str, char *buf_str, int len)
{
	int		index;
...
static char	*form_nl(char *next_line, char *buffer, char *newline)
{
	...
	if (newline)
	{
		new_string = concatenate(new_string, next_line, buffer, len);
		buffer = newline + 1;
